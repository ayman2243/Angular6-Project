...
export class DeleteComponent extends MzBaseModal implements OnInit {
  ...
  delete(){
    this._title.getTitle(this['id']).subscribe(
      (tl) =>{
        this._title.deleteTitle(this['id'], localStorage.getItem('token')).subscribe(
          (done) => {
            $('.modal-overlay, app-delete').remove();
            this.router.navigateByUrl("/dashboard/titles/list?d="+this.id);
          }
        )
    },
    (err) =>{
